%%%%%%%%%%%%%%%%% PREAMBLE %%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Change the font size of your document - 10pt, 12.1pt, etc.
\documentclass[letterpaper,11pt,oneside]{article}
\usepackage[utf8]{inputenc}
\usepackage[T1]{fontenc}
\usepackage{setspace}
\usepackage{scrextend}
\usepackage{hyperref}
\usepackage{comment}
\usepackage{color}
\definecolor{blue}{RGB}{32,71,150}
\usepackage{charter} %Helvetica font
\usepackage[none]{hyphenat} %Removes hyphenation
\usepackage{enumitem} % removes itemization vspace

% Sets itemize style
\renewcommand{\labelitemi}{\textendash}
\setlist[itemize]{noitemsep, topsep=0pt}

% Creates Heading function
\newcommand{\createHeading}[1]{
    \vspace{1.25em}
    \hline
    \vspace{-1.5ex}
    \begin{center}
        #1
    \end{center}
    \vspace{-1.5ex}
    \hline
    \vspace{1.25em}
}

%Change the margins to fit your CV/resume content
\usepackage[left=.6in, right=.6in, bottom=.6in, top=.6in]{geometry}

%Changes the page numbers - {arabic}=arabic numerals, {gobble}=no page numbers, {roman}=Roman numerals
\pagenumbering{gobble}

%%%%%%%%%%%%%%%%% END OF PREAMBLE %%%%%%%%%%%%%%%%%%%%%



\begin{document}

%%%%%%%%%%%%%%%%% HEADER %%%%%%%%%%%%%%%%%%%%%%%%%%%

\noindent \huge{\textbf{\textcolor{blue}{\href{https://www.urbandictionary.com/define.php?term=Nathan}{Nathan}} Cheung}} \hfill \normalsize{\href{https://nathancheung.site}{nathancheung.site} | \href{mailto:nathancheung22@gmail.com}{nathancheung22@gmail.com}}



%%%%%%%%%%%%%%%%% MAIN BODY %%%%%%%%%%%%%%%%%%%%%%%%%%%

% EDUCATION
\createHeading{EDUCATION}
\begin{addmargin}[.6em]{.6em}
    \textbf{Ryerson University} \\
    Bachelor of Engineering in Software Engineering \hfill Exp. Apr 2022 \\
    Dean's List: 2018 - 2020 \hfill CGPA: \textbf{3.82}
\end{addmargin}

% PROJECTS
\createHeading{PROJECTS}
\begin{addmargin}[.6em]{.6em}
    % Vidshare
    \textbf{Youtube Video Synchronizer} \hfill \href{https://vidshare.ca/}{https://vidshare.ca/}
    \begin{itemize}
        \item Developed a full-stack web application to synchronize YouTube Playback in real time
        \item Synchronized all user activity using \textbf{Socket.io websockets} and \textbf{Express.js} with back-end API endpoints
        \item Designed front-end UI using \textbf{HTML, CSS, JavaScript,} and \textbf{Bootstrap}
    \end{itemize}
    \vspace{1.25em}
    % Hand written digits
    \textbf{Handwritten Digits Classifier} \hfill \href{https://handwritten-digits-ai.netlify.app/}{http://tiny.cc/izm5tz}
    \begin{itemize}
        \item Created and trained a \textbf{neural network} in \textbf{Keras} to classify handwritten digits
        \item Designed a full-stack web application to visualize neuron layer with \textbf{Node.js, React,} and \textbf{Express.js} 
    \end{itemize}
    \vspace{1.25em}
    % Kijiji scraper
    \textbf{Automated Classifieds Bot} \hfill \href{https://git.io/JkKfz}{https://git.io/JkKfz}
    \begin{itemize}
        \item Created a \textbf{Python} bot to identify high-margin listings and notify user via email
        \item Utilized \textbf{web scraping} with \textbf{Beautiful Soup} and \textbf{Pandas} to scrape listings and store data dynamically
        \item Automated and deployed bot using an \textbf{AWS EC2} instance and email notifications with the \textbf{smtp} library
    \end{itemize}
    \vspace{1.25em}
    % Password Manager
    \textbf{Password Manager} \hfill \href{https://git.io/JkKk6}{https://git.io/JkKk6}
    \begin{itemize}
        \item Created a \textbf{Python} application to store and retrieve passwords securely
        \item Ensured security using \textbf{bcrypt}'s Blowfish hashing algorithm with a randomly generated salt
        \item Encrypted passwords with \textbf{AES-128-CBC encryption} and \textbf{PBKDF2's HMAC-SHA-256} key hashing
    \end{itemize}
    \vspace{1.25em}
    % Sudoku Solver
    \textbf{Sudoku Solver} \hfill \href{https://git.io/JkKJO}{https://git.io/JkKJO}
    \begin{itemize}
        \item Created a user-friendly application that solve Sudoku puzzles
        \item Implemented a \textbf{backtracking} algorithm with visualization using \textbf{pygame}
    \end{itemize}
\end{addmargin}

% EXPERIENCE
\createHeading{EXPERIENCE}
\begin{addmargin}[.6em]{.6em}
    % IBM
    \textbf{Software Engineer Intern} \\
    \textit{IBM Canada} \hfill \textit{May 2020 - Present}
    \begin{itemize}
        \item Developed a full-stack web application using \textbf{TypeScript, React/Redux,} and \textbf{Express.js}
        \item Constructed an end-to-end workflow testing suite to improve error catching by 70\%
        \item Built a data scraping Slack Bot using \textbf{Python, Selenium,} and \textbf{Slack API}
        \item Automated an internal data analytics process using \textbf{Node.js} and \textbf{Chart.js} for visualization
        % \item Outlined design documentation for long term maintenance and support
        \item Deployed all applications in \textbf{Docker Containers} using \textbf{CI/CD pipelines} with \textbf{Travis CI}
    \end{itemize}
    \vspace{1.25em}
    % TD
    \textbf{Customer Experience Associate} \\
    \textit{TD Canada Trust} \hfill \textit{Aug 2019 - Apr 2020}
    \begin{itemize}
        \item Granted the additional responsibility to handle secure-desk and bank vault combinations
        \item Complied with branch control procedures to prevent identity theft, money laundering, and fraud
    \end{itemize}
\end{addmargin}

% TECHNICAL SKILLS
\createHeading{TECHNICAL SKILLS}
\begin{addmargin}[.6em]{.6em}
    \textbf{Languages: } Python, JavaScript, Java, HTML, CSS, C, SQL \\
    \textbf{Tools/Technologies: } Git, Node.js, TypeScript, Docker, AWS, Express.js, Travis CI, JIRA, MongoDB, Keras
\end{addmargin}
\end{document}
